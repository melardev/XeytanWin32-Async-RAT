		...
		if (isInteracting())
		{
			...
			if (strlen(instruction) > 2)
			{
				...
				else
				{
					...
					size_t pos = instructionStr.find(L" ");
...
void AppConsole::setInteractionView(Client* client)
{
	...
	std::string prompt = getPromptMessage(false);
	::printf("%s/%llu>$ ", prompt.c_str(), client->getId());
}
...
void AppConsole::showProcesses(Client* client, std::vector<Process>* processes)
{
	...
	for (int i = 0; i < processes->size(); i++)
	{
		...
		size_t index = process.path.find_last_of('\\');
